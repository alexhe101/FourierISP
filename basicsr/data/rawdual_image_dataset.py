...
class RAWDualImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(RAWDualImageDataset, self).__init__()
        self.opt = opt
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = tripled_paths_from_lmdb([self.lq_folder, self.gt_folder], ['lq', 'gt'])
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            # flip, rotation
            img_gt,img_combined,img_demosaic = numpyaug( [img_gt,img_combined,img_demosaic], self.opt['use_hflip'], self.opt['use_rot'])
        ...
        # img_demosaic = img_demosaic.unsqueeze(0)
        # img_combined = img_combined.unsqueeze(0)
        # _,c,h,w = img_demosaic.size()
        # nh,nw = h//4,w//4
        # nh,nw = nh//16,nw//16
        # nh,nw = nh*16,nw*16

        # img_demosaic = F.interpolate(img_demosaic,size=(nh,nw),mode='bilinear')
        # img_combined = F.interpolate(img_combined,size=(nh//2,nw//2),mode='bilinear')
        # img_demosaic= img_demosaic.squeeze(0)
        # img_combined= img_combined.squeeze(0)

        # normalize
        if self.mean is not None or self.std is not None:
            normalize(img_lq, self.mean, self.std, inplace=True)
            normalize(img_gt, self.mean, self.std, inplace=True)
        return {'combined': img_combined, 'gt': img_gt, 'lq_path': lq_path, 'gt_path': gt_path,'demosaic':img_demosaic}
