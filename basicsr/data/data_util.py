...
def tripled_paths_from_lmdb(folders, keys):
    ...
    if set(input_lmdb_keys) != set(gt_lmdb_keys) or set(gt_lmdb_keys)!=set(mask_lmdb_keys):
        raise ValueError(f'Keys in {input_key}_folder and {gt_key}_folder are different.')
    else:
        paths = []
        for lmdb_key in sorted(input_lmdb_keys):
            paths.append(dict([(f'{input_key}_path', lmdb_key), (f'{gt_key}_path', lmdb_key), (f'{mask_key}_path', lmdb_key)]))
        return paths
...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    for gt_path in gt_paths:
        ...
        input_name = f'{filename_tmpl.format(basename)}.png'  # 修改文件名为.png后缀
        # input_name = f'{filename_tmpl.format(basename)}{ext}'
        input_path = osp.join(input_folder, input_name)
...
def tri_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(keys) == 3, (
        ...
        f'But got {len(keys)}')
    input_folder, gt_folder,semantic_folder = folders
    input_key, gt_key,sem_key = keys

    input_paths = list(scandir(input_folder))
    gt_paths = list(scandir(gt_folder))
    sem_paths = list(scandir(semantic_folder))
    ...
    for idx in range(len(gt_paths)):
        ...
        input_path = osp.join(input_folder, input_name)
        sem_path = osp.join(semantic_folder,input_name)
