...
class RGBuvHistBlock(nn.Module):
  def __init__(self, h=128, insz=448, resizing='interpolation',
               method='inverse-quadratic', sigma=0.02, intensity_scale=True,
               hist_boundary=None, green_only=False):
...
class HistLoss(nn.Module):
    ...
    def forward(self,pred,target):
        #这个是用来做3个unet的loss
        pred_hist = self.histogram_block(pred)
        ...
        histogram_loss = (1/np.sqrt(2.0) * (torch.sqrt(torch.sum(
            torch.pow(torch.sqrt(target_hist) - torch.sqrt(pred_hist), 2)))) / 
                pred_hist.shape[0])
        return self.loss_weight*histogram_loss