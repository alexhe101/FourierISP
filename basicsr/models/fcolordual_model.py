...
class FcolorDualModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(FcolorDualModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.out_1_phase, self.out_1_amp, self.out_2 = self.net_g(self.combined,self.demosaic)
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.out_1_phase, self.out_1_amp, self.out_2 = self.net_g_ema(self.combined,self.demosaic)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.out_1_phase, self.out_1_amp, self.out_2 = self.net_g(self.combined,self.demosaic)
