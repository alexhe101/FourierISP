...
class FpnAlignModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(FpnAlignModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        if 'demosaic' in data:
            self.lq = data['demosaic'].to(self.device)
        else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.out_1, self.out_1_amp, self.out_1_phase, self.out_2 = self.net_g(self.lq)

        l_total = 0
        loss_dict = OrderedDict()
        # pixel loss
        if self.cri_pix:
            l_pix = self.cri_pix(self.out_2*self.mask, self.gt*self.mask)
            l_total += l_pix
        ...
        if self.cri_ssim:
            l_ssim = self.cri_ssim(self.out_2*self.mask, self.gt*self.mask)
            l_total+=l_ssim
            loss_dict['l_ssim'] = l_ssim
        if self.cri_vgg:
            l_vgg = self.cri_vgg(self.out_2*self.mask,self.gt*self.mask)
            l_total+=l_vgg
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.out_1, self.out_1_amp, self.out_1_phase, self.out_2 = self.net_g_ema(self.lq)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.out_1, self.out_1_amp, self.out_1_phase, self.out_2 = self.net_g(self.lq)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            sr2_img = tensor2img([visuals['result']])
