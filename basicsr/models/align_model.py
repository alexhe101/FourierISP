...
def normalize_gray_image(gray_image):
    # 计算最小值和最大值
    min_val = torch.min(gray_image)
    max_val = torch.max(gray_image)
    
    # 标准化灰度图像
    normalized_gray_image = (gray_image - min_val) / (max_val - min_val)
...
class AlignModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_pix:
            ...
            # l_pix = self.cri_pix(grayscale_transform(self.output),normalize_gray_image(grayscale_transform(self.lq)))
            l_total += l_pix
