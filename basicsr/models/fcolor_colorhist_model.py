...
class FcolorHistModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(FcolorHistModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.out_1_phase, self.out_1_amp, self.out_1,self.out_2 = self.net_g(self.lq)
        ...
        if self.cri_hist:
            l_hist = self.cri_hist(self.out_1,self.gt)
            l_total+=l_hist
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.out_1_phase, self.out_1_amp,self.out_1, self.out_2 = self.net_g_ema(self.lq)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.out_1_phase, self.out_1_amp, self.out_1,self.out_2 = self.net_g(self.lq)
